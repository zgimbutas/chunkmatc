...
c
c
        call l2getrhs(rk,source,xys,rhs,npts)
c
        call prinf('npts=*',npts,1)
c
c       ... add the diagonal term
c
        if( ifexterior .eq. 0 ) d=+pi
        if( ifexterior .eq. 1 ) d=-pi
        call chunkdiag(cmatr,npts,d)
c
        ifprec = 1
c
        if( ifprec .eq. 1 ) then
...
c
     ...
     $     l2dmulta,p1,p2,p3,p4,p5,p6,rhs,eps,numit,
...
c
        ...
        enddo
c
        call prin2('capacitance, integral of the solution=*',cd,2)
        call prin2('capacitance-1=*',cd-1,2)
c
        call l2soleva_dp(rk,target,xys,xynorms,sol,whts,npts,cpot)
        call prin2('cpot=*',cpot,2)
c
        call l2direva(rk,source,target,cpot0)
...
c
c
        subroutine l2getrhs(rk,source,xys,rhs,npts)
        implicit real *8 (a-h,o-z)
...
c
c
        subroutine l2soleva(rk,target,xys,sol,whts,npts,cpot)
        implicit real *8 (a-h,o-z)
...
c
        ...
        r=sqrt(dx**2+dy**2)
c
        cd=(dx*xynorms(1,i)+dy*xynorms(2,i))/r**2
...
c
c
        subroutine l2direva(rk,source,target,cpot)
        implicit real *8 (a-h,o-z)
        dimension source(2),target(2)
...
c
c
        subroutine l2dmulta(a,p1,p2,p3,p4,p5,p6,x,y,n)
        implicit real *8 (a-h,o-z)
...
c
c
        subroutine l2dmultb(a,p1,p2,p3,p4,p5,p6,x,y,n)
        implicit real *8 (a-h,o-z)
        complex *16 a(n,n),x(n),y(n)
c
        do 1200 i=1,n
