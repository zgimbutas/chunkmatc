...
c
        ...
        complex *16, allocatable :: dmatr(:,:)
        complex *16, allocatable :: rhs(:)
        complex *16, allocatable :: rhsneu(:)
        complex *16, allocatable :: sol(:)
        complex *16, allocatable :: d2n(:)
        complex *16, allocatable :: sol3(:)
c
...
c
        ...
        allocate( dmatr(npts,npts) )
        allocate( rhs(npts) )
        allocate( rhsneu(npts) )
        allocate( sol(npts) )
        allocate( d2n(npts) )
        allocate( sol3(npts) )
c
...
c
        ...
        call h2getrhsneu(rk,source,xys,xynorms,rhsneu,npts)
c
        call prinf('npts=*',npts,1)
c
...
c
        ...
        call chunkdiag_cmplx(cmatr,npts,cd)
c
        ifprec = 1
c
        if( ifprec .eq. 1 ) then
...
c
        ...
        do i=1,npts
        do j=1,npts
        cmatr(i,j)=cmatr(i,j)*(whts(i))/(whts(j))
        enddo
        rhs(i)=rhs(i)*(whts(i))
        enddo
        endif
...
c
        call h2soleva_dp(rk,target,xys,xynorms,sol,whts,npts,cpot)
...
c
        ...
        call h2dmulta(dmatr,p1,p2,p3,p4,p5,p6,sol,d2n,npts)

        call prin2('D_k prime(sol)=*',d2n,2*12)
        call prin2('while rhsneu=*',rhsneu,2*12)


        do i = 1,npts
        sol3(i) = (d2n(i) - rhsneu(i))/rhsneu(i)
        enddo
        
        call prin2('relative errors in D-N map=*',sol3,2*12)
        ...
        call prin2('L2 relative error in D-N map=*',d,1)

        do i = 1,npts
        sol3(i) = (d2n(i) - rhsneu(i))
        enddo
        
        call prin2('absolute errors in D-N map=*',sol3,2*12)
        ...
        call prin2('L2 absolute error in D-N map=*',d,1)
...
c
c
        subroutine h2getrhsneu(rk,source,xys,xynorms,rhs,npts)
        implicit real *8 (a-h,o-z)
        dimension source(2),xys(2,1),xynorms(2,1)
        complex *16 rhs(npts),rk,ima,z,h0,h1,cd
        data ima/(0.0d0,1.0d0)/
c
        done=1
        pi=4*atan(done)
...
c
c
        subroutine h2soleva_dp
     $     (rk,target,xys,xynorms,sol,whts,npts,cpot)
        implicit real *8 (a-h,o-z)
        dimension target(2),xys(2,1),xynorms(2,1),whts(1)
        complex *16 sol(npts),cpot,rk,ima,z,h0,h1,cd
        data ima/(0.0d0,1.0d0)/
c
        cpot=0
        do 1200 i=1,npts
...
c
c
        subroutine chunkdiag_cmplx(cmatr,npts,cd)
        implicit real *8 (a-h,o-z)
        complex *16 cmatr(npts,npts),cd
c
        do 1200 i=1,npts
        cmatr(i,i)=cmatr(i,i)+cd
 1200   continue
